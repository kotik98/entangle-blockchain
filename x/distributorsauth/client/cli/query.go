...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				distributorAddress, err := types.EnsureBech32Address(args[0])
				if err != nil {
					return err
				}
				query.DistributorAddr = distributorAddress
			}
...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				adminAddress, err := types.EnsureBech32Address(args[0])
				if err != nil {
					return err
				}
				query.AdminAddr = adminAddress
			}
